...
the YANG "rpc" statement MUST be invoked with the POST method.
...
manner.   This strategy does not obviate the need for responses provided
by the server to contain HATEOAS links, originally described in
Roy Fielding's doctoral dissertation ^rest-dissertation^.

A general-purpose REST client using HATEOAS principles would not use
...
to discover the server capabilities.  HATEOAS presents a few
advantages with regards to control of complex networking devices:

- A general-purpose REST client which is not schema-aware can still be used to navigate the resources
and their corresponding relationships.

- A developer or a test engineer can still write client code efficiently without always needing to consult the schema or documentation.
Furthermore, the resulting code will be more laconic and efficient, because there is no need to "construct" URIs using foreign key fields.

- Dynamic (data-dependant) links cannot be efficiently supported by the schema: for example, a shutdown-device rpc link which only appears when the state of the device
is 'up'.

- Heterogeneous links which may reference different resources are supported with ease.

The RESTCONF response does not, by default, return HATEOAS links, unless the "hateoas" query parameter is provided by the client in the request URL.
...
- multipart/form-data, image/png, image/jpg or application/octet-stream [RFC 1867]: Do we need these???  These are standard protocols for uploading/downloading binary data.
...
MUST be sent by the client in the request, otherwise the request
message MUST NOT include a message body.

If the "rpc" statement has an "output" section, then a message body
MUST be sent by the server in a successful response (response with the Status-Line less than 300). 
If the "rpc" statement does not have an "output" section, the
server MUST NOT include a message body in a successful response message,
...
How does the client discover this version of the resource???
...
the client, depending on whether the server wishes to make the reason
for refusal available to the client.  For most cases, the "403 Forbidden" Status-Line
is the more common response.
...
What if the created resource is different than the request message body???
User may still want to see the actual created resource.  Also, in case of PUT
we do allow response body, so we should also allow it for POST.
...
If the POST request body contains nodes which do not exist in the YANG schema,
...
content used to replace the target resource.
...
If the resource to be replaced does not exist, a "404 Not Found" is returned.

If the PUT request body contains nodes which do not exist in the YANG schema,
a "400 Bad Request" is returned.
...
is replaced if it does not already exist.
...
If the resource to be patched does not exist, a "404 Not Found" is returned.

If the PATCH request body contains nodes which do not exist in the YANG schema,
or a child resource does not exist in the target resource, a "400 Bad Request" is returned.
...
If the resource to be deleted does not exist, a "404 Not Found" is returned.
...
!! row  ! insert ! POST ! Insertion mode for user-ordered data resources
!! row  ! limit ! GET ! Number of entries to return for collection resources
!! row  ! offset ! GET ! Starting point for collection resources
!! row  ! point ! POST ! Insertion point for user-ordered data resources
...
!! row  ! hateoas ! GET, PUT, POST ! Show HATEOAS links on output
...
This parameter is only supported for the POST
method. It is also only supported if the target
...
or equal to 1, or the string "unbounded".  The default value is 1000.
This is required not to overload the server if the data set is too large.
...
How is nullable handled in RESTCONF?
...
What about multipart/form-data, image/png, image/jpg or application/octet-stream [RFC 1867]: Do we need these???  These are standard protocols for uploading/downloading binary data.
...
We must also support CDATA.  YANG's "anyxml" statement should always map to CDATA in RESTCONF.
CDATA can be specified for both input and output messages.
...
We also need to support notifications of changes to resources.  This can be useful
for supporting notifications for GUI clients.
...
The server might respond:  (but DELETE method didn't specify yang.errors in Accept???)
...
- HATEOAS
- How to retrieve a particular version of a resource?
- Asynchronous, Long Running Requests (LRR).
- PATCH is missing insert and delete.
- Notifications on changes to particular resource.
- Multipart/form-data, images.
- Sorting.
- Access domains.
- CDATA.
- Metadata/Info service doesn't return error code information.
- Snapshot.
